...
const PropertyList = () => {
  ...
  const handleDeleteListing = async (id) => {
    try {
      ...
      if (response.ok) {
        // Remove deleted listing from the local state
        dispatch(setPropertyList(propertyList.filter((item) => item._id !== id)));
      ...
      <div className="list">
        {propertyList?.map(
          ({
            ...
            booking={booking}
            showDeleteIcon={true} 
            onDelete={() => handleDeleteListing(_id)} // Handle delete action
